...
<div class="mb-4">
    ...
    <nav aria-label="breadcrumb">
        <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item active">ویرایش: {{ $product->name }}</li>
        </ol>
    </nav>
</div>
        ...
        <div class="row g-3">
            <div class="col-md-6 col-lg-3">
                ...
                <select name="product_type" class="form-select @error('product_type') is-invalid @enderror">
                    <option value="normal"
                        {{ old('product_type', $product->product_type ?? 'normal') == 'normal' ? 'selected' : '' }}>
                        معمولی
                    </option>
                    <option value="injection"
                        {{ old('product_type', $product->product_type ?? '') == 'injection' ? 'selected' : '' }}>
                        تزریق
                    </option>
                </select>
            ...
            <div class="col-md-6 col-lg-3">
                ...
                <select name="firing_process" class="form-select @error('firing_process') is-invalid @enderror">
                    <option value="tonneli"
                        {{ old('firing_process', $product->firing_process) == 'tonneli' ? 'selected' : '' }}>
                        تونلی
                    </option>
                    <option value="shuttle"
                        {{ old('firing_process', $product->firing_process) == 'shuttle' ? 'selected' : '' }}>
                        شاتل
                    </option>
                    <option value="both"
                        {{ old('firing_process', $product->firing_process) == 'both' ? 'selected' : '' }}>
                        هر دو
                    </option>
                </select>
            ...
            <div class="col-md-6 col-lg-3">
                ...
                <input type="number" name="cavities"
                       ...
                       value="{{ old('cavities', $product->cavities) }}" min="1">
            ...
            <div class="col-md-6 col-lg-3">
                ...
                <input type="number" name="tonneli_feed_rate"
                       ...
                       value="{{ old('tonneli_feed_rate', $product->tonneli_feed_rate) }}" min="0">
                ...
                <select name="formula_id" class="form-select @error('formula_id') is-invalid @enderror">
                    ...
                    @foreach($formulas as $formula)
                        <option value="{{ $formula->id }}"
                            {{ old('formula_id', $product->formula_id) == $formula->id ? 'selected' : '' }}>
                            {{ $formula->name }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="col-md-6 col-lg-6">
                ...
                <select name="parent_product_id"
                        class="form-select @error('parent_product_id') is-invalid @enderror">
                    <option value="">بدون والد (خام)</option>
                    @foreach($allProducts as $p)
                        <option value="{{ $p->id }}"
                            {{ old('parent_product_id', $product->parent_product_id) == $p->id ? 'selected' : '' }}>
                            {{ $p->name }}
                        </option>
                    @endforeach
                </select>
                ...
                @if($product->parent)
                    <small class="text-muted">
                        ...
                        والد فعلی: <strong>{{ $product->parent->name }}</strong>
                    </small>
                @endif
            </div>
        </div>
        ...
        <div class="row g-3">
                    ...
                    <div class="row g-3">
                            ...
                            <select name="carton_packaging_id"
                                ...
                                @foreach($packagings as $packaging)
                                    <option value="{{ $packaging->id }}"
                                        {{ old('carton_packaging_id', $product->carton_packaging_id) == $packaging->id ? 'selected' : '' }}>
                                        {{ $packaging->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-12">
                            ...
                            <input type="number" name="per_box"
                                   ...
                                   value="{{ old('per_box', $product->per_box) }}" min="0"
                    ...
                    <div class="row g-3">
                        <div class="col-12">
                            ...
                            <select name="layer_packaging_id"
                                ...
                                @foreach($packagings as $packaging)
                                    <option value="{{ $packaging->id }}"
                                        {{ old('layer_packaging_id', $product->layer_packaging_id) == $packaging->id ? 'selected' : '' }}>
                                        {{ $packaging->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-12">
                            ...
                            <input type="number" name="layers_per_box"
                                   ...
                                   value="{{ old('layers_per_box', $product->layers_per_box) }}" min="0"
                                   placeholder="مثلاً ۳">
            ...
            <div class="col-md-6">
                ...
                <input type="number" name="per_pack"
                       ...
                       value="{{ old('per_pack', $product->per_pack) }}" min="0">
            ...
            <div class="col-md-6">
                ...
                <input type="number" name="per_pallet"
                       ...
                       value="{{ old('per_pallet', $product->per_pallet) }}" min="0">
                ...
                <div class="checkbox-card">
                    <div class="form-check">
                        <input type="checkbox" name="in_production" class="form-check-input" id="in_production" value="1"
                               {{ old('in_production', $product->in_production) ? 'checked' : '' }}>
        ...
        <div class="row g-3">
            <div class="col-12">
                <input type="text" name="aliases"
                       ...
                       value="{{ old('aliases', $product->aliases->pluck('alias')->implode('، ')) }}"
                ...
                @if($product->aliases->count())
                    <div class="current-alias-box">
                        ...
                        @foreach($product->aliases as $alias)
                            <span class="badge bg-info me-1">{{ $alias->alias }}</span>
                        @endforeach
                    </div>
                @else
                    <div class="current-alias-box">
