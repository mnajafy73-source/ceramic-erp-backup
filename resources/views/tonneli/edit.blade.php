...
<script>
    let itemIndex = {{ $tonneli->items->count() }};
            ...
            <div class="row g-2 mb-2 item-row" id="item-row-${itemIndex}">
                ...
                <div class="col-md-3 d-flex align-items-center">
                    <div class="form-check form-switch">
                        ...
                        <input class="form-check-input packaged-toggle" type="checkbox" name="items[${itemIndex}][is_packaged]" value="1" ${checked}>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        // مقداردهی Select2 برای ردیف‌های موجود
        ...
        @foreach($tonneli->items as $item)
            addItemRow({{ $item->product_id }}, '{{ $item->input_quantity }}', '{{ $item->output_quantity }}', {{ $item->is_packaged ? 'true' : 'false' }});
        @endforeach
    });
</script>
@endpush
            ...
            <div class="row">
                <div class="col-md-6 mb-3">
                    ...
                    <input type="text" name="date" id="date" class="form-control @error('date') is-invalid @enderror" 
                           value="{{ old('date', $tonneli->jalali_date) }}" required autocomplete="off">
